...
async def get(
    ...
    result = db.statements.get_many(form_data,current_user)
    return result
...
async def create(
    ...
    result = db.statements.create(form_data, current_user)
    return result
...
async def delete_globally(
    ...
    result = db.statements.delete_globally(form_data, current_user)
    return result
...
async def delete_for_project(
        ...
        form_data: Annotated[requests.SearchRequest, Depends()]):
    print(f"DELETE STATEMENT \nBy: {current_user}\nBody: {form_data}")
    result = db.statements.delete_for_project(form_data, current_user)
    return result